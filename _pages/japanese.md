---
layout: archive
title: "Japanese"
permalink: /japanese/
author_profile: true
---

{% include base_path %}

## Profile
奈良先端科学技術大学院大学 先端科学技術研究科 博士前期課程に所属しています。

ソーシャル・コンピューティング研究室にて、医療言語処理に関する研究に取り組んでいます。

## Interests
* AIの医療応用
* 医療自然言語処理
* マルチモーダルモデルの医療応用

## Education
* 2025.04 - 現在: 奈良先端科学技術大学院大学 先端科学技術研究科 先端科学技術専攻 情報科学領域
  * ソーシャル・コンピューティング研究室
  * 博士前期課程（2027.03 卒業見込）

* 2023.09 - 2024.08: Technical University of Munich, CIT-Informatics
  * 交換留学
  * 修士課程コースに所属

* 2020.04 - 2025.03: 九州大学 工学部 機械航空工学科
  * 生体機能工学研究室
  * 学士課程卒業

* 2016.04 - 2019.03: 福岡県立修猷館高等学校


## Publications

### International Conference
* <u>Yuki Tashiro</u>, Seiji Shimizu, Tomohiro Nishiyama, Shoko Wakamiya, and Eiji Aramaki. "MedNormJ: A Benchmark Dataset for Medical Concept Normalization in Japanese Clinical Documents." Proceedings of the 8th Clinical Natural Language Processing Workshop (ClinicalNLP 2026, Workshop), 2026/05 (<span style="color: tomato; ">**Oral**</span>, 18% of 40 submissions).
* Yuta Nakamura, Koji Fujimoto, Jonas Kluckert, Michael Krauthammer, Jun Kanzawa, Akira Katayama, Tomohiro Kikuchi, Ryo Kurokawa, Wataru Gonoi, Peitao Han, <u>Yuki Tashiro</u>, Suntaro Yada, and Eiji Aramaki. "NTCIR-18 RadNLP 2024 Overview: Dataset and Solutions for Automated Lung Cancer Staging." Proceedings of the NTCIR-18 Conference, 2025.
* <u>Yuki Tashiro</u>, Yuta Nakamura, and Eiji Aramaki. "SOCIOCOM at the NTCIR-18 RadNLP Main Task: Zero-Shot LLM Approaches for Lung Cancer Staging." NII Institutional Repository, 2025.

## Domestic Conference
* <u>田代 勇希</u>, 清水 聖司, 西山 智弘, 若宮 翔子, 荒牧 英治. MedNormJ：日本語医療テキストにおける病名正規化のための文脈付きデータセットの構築. NLP2026, 2026/03.
* <u>田代 勇希</u>, 清水 聖司, 久田 祥平, 西山 智弘, 若宮 翔子, 荒牧 英治. 日本語医療用語に対する様々な正規化手法の評価. 第265回自然言語処理研究会（NL研）, 2025/09.


## Internship
* 2025.09 - 現在: Direava 株式会社
  * NEDO「GENIAC」プロジェクト「外科手術支援のための視覚・言語統合型AI基盤モデルの開発」に従事
  * VLMのモデル開発を担当

* 2025.08 - 2025.09: GMOインターネットグループ株式会社
  * 数百万件規模の決済取引データを対象に不正検知を機械学習で実施

* 2024.11 - 2024.12: 奈良先端科学技術大学院大学 ソーシャル・コンピューティング研究室
  * 医療テキストデータに対する辞書ベース固有表現抽出をRAG等を用いて検証
  * 国際学会 Shared Task のベースライン公開と論文執筆

* 2023.12 - 現在: 株式会社 松尾研究所
  * ウェアラブルデバイスデータ解析に関する共同研究

* 2023.05 - 2025.03: 株式会社 松尾研究所
  * 医療AIに関する共同研究に従事
  * アーキテクチャに関する基礎研究

## Awards
* 第20回言語処理若手シンポジウム (YANS2025) コンペティション 2位
* 第6回眼科AI学会総会 AIコンペティション 8位
* Fine-tuning RAG Chatbots on your own data Competition, 2nd Prize
* AI for Drug Development with PHIO Competition, 2nd Prize
* 宝くじ仮説を用いた Image Inpainting コンペティション 4位・ディスカッション賞
* トビタテ！留学 JAPAN 日本代表プログラム 第15期 採択

## Skills
* Languages: Python, Linux, C

* 機械学習・開発環境
  * PyTorch, Hugging Face Transformers, Docker, Git, Azure, AWS, Weights & Biases, Streamlit, MySQL

* 計算資源・実装経験
  * 複数ノードでのVision Language Modelの分散学習

* 医療データ解析
  * テキスト（読影レポート、論文等）
  * 画像（MRI、CT、心電図、顕微鏡画像、眼底画像、手術画像等）
  * 生体時系列データ（ECG、PPG、ウェアラブルセンサ等）
  * テーブルデータ
  * 動画（手術動画等）

## 語学・資格
* IELTS 5.0 (2022.09)
* スポーツ医学検定 2級 (2021.01)
* 世界遺産検定 2級 (2021.01)
* 第一種自動車免許 (2021.07)
* 中国語検定準4級 (2021.08)
